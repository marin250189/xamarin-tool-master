...
namespace RavSoft.GoogleTranslator
{
	public class Translator
	{
		...
		void DoSomethingOnFinish(object sender, AsyncCompletedEventArgs e)
		{
			...
			string outputFile = ((System.Net.WebClient)(sender)).QueryString["outputFile"];
			string text = File.ReadAllText(outputFile);
			int index = text.IndexOf(string.Format(",,\"{0}\"", Translator.LanguageEnumToIdentifier("es")));
			if (index == -1)
			{ }
			 ...
			 string targetLanguage)
		{
			...
			try
			{
				...
				string pathToNewFolder = Path.Combine(Android.OS.Environment.ExternalStorageDirectory.AbsolutePath, "XF_Downloads");
				//Directory.CreateDirectory(pathToNewFolder);
				string outputFile = Path.Combine(pathToNewFolder, "json.txt");
				...
				using (WebClient wc = new WebClient())
				{
					...
					wc.Headers.Add("user-agent", "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36");
					 ...
					 await Task.Run( () => {  wc.DownloadFile(url, outputFile); });
